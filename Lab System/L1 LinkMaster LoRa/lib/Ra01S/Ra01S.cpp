...
int16_t SX126x::begin(uint32_t frequencyInHz, int8_t txPowerInDbm, float tcxoVoltage, bool useRegulatorLDO)
{
  ...
  if (syncWord != SX126X_SYNC_WORD_PUBLIC && syncWord != SX126X_SYNC_WORD_PRIVATE) {
    if (debugPrint) Serial.println("SX126x error, maybe no SPI connection");
...
void SX126x::FixInvertedIQ(uint8_t iqConfig)
{
  ...
  // Standard IQ: bit 2 = 1, Inverted IQ: bit 2 = 0
  if(iqConfig == SX126X_LORA_IQ_STANDARD) {
    iqConfigCurrent |= 0x04;   // Set bit 2 for standard IQ
  } else {
    iqConfigCurrent &= 0xFB;   // Clear bit 2 for inverted IQ
  }
...
void SX126x::LoRaConfig(uint8_t spreadingFactor, uint8_t bandwidth, uint8_t codingRate, uint16_t preambleLength, uint8_t payloadLen, bool crcOn, bool invertIrq)
{
  ...
  // Calculate LDRO: enable for SF10+ at 125kHz (helps with timing/drift)
  // BW: 4=125kHz, 5=250kHz, 6=500kHz
  uint8_t ldro = 0;
  if (bandwidth == 4) { // 125 kHz
    if (spreadingFactor >= 10) ldro = 1;  // Enable for SF10, SF11, SF12
  } else if (bandwidth == 5) { // 250 kHz
    if (spreadingFactor >= 11) ldro = 1;  // SF11, SF12
  }
  // 500 kHz: only SF12 might need it
  else if (bandwidth == 6 && spreadingFactor == 12) {
    ldro = 1;
  }

  if (debugPrint) Serial.printf("[LORA] SF%d BW%d LDRO=%d\n", spreadingFactor, bandwidth, ldro);
  ...
  // PacketParams[4] = CRC mode (0x00=OFF, 0x01=ON)
  if ( crcOn )
    PacketParams[4] = SX126X_LORA_CRC_ON;
  else
    PacketParams[4] = SX126X_LORA_CRC_OFF;

  // PacketParams[5] = IQ polarity (0x00=Standard, 0x01=Inverted)
  if ( invertIrq )
...
uint8_t SX126x::Receive(uint8_t *pData, uint16_t len)
{
  ...
  if( irqRegs & SX126X_IRQ_RX_DONE )
  {
    ...
    // Check for CRC error - reject packet if CRC failed
    if( irqRegs & SX126X_IRQ_CRC_ERR )
    {
      return 0;  // CRC error, discard corrupted packet
    }
...
bool SX126x::Send(uint8_t *pData, uint8_t len, uint8_t mode)
{
  ...
  if ( txActive == false )
  {
    ...
    SetTx(15000);  // 15 second timeout (enough for SF12 + 255 bytes)
...
void SX126x::Reset(void)
{
  ...
  WaitForIdle(BUSY_WAIT, (char*)"Reset", true);
}
...
void SX126x::SetDio3AsTcxoCtrl(float voltage, uint32_t delayTime)
{
  uint8_t buf[4];
  ...
  uint32_t delayValue = (float)delayTime / 15.625;
...
uint8_t SX126x::ReadBuffer(uint8_t *rxData, uint8_t maxLen)
{
  ...
  // ensure BUSY is low (state meachine ready)
  WaitForIdle(BUSY_WAIT, (char*)"start ReadBuffer", true);
  ...
  WaitForIdle(BUSY_WAIT, (char*)"end ReadBuffer", false);
...
{
  // ensure BUSY is low (state meachine ready)
  WaitForIdle(BUSY_WAIT, (char*)"start WriteBuffer", true);
  ...
  // wait for BUSY to go low
  WaitForIdle(BUSY_WAIT, (char*)"end WriteBuffer", false);
}


void SX126x::WriteRegister(uint16_t reg, uint8_t* data, uint8_t numBytes, bool waitForBusy) {
  // ensure BUSY is low (state meachine ready)
  WaitForIdle(BUSY_WAIT, (char*)"start WriteRegister", true);
  ...
  // wait for BUSY to go low
  if(waitForBusy) {
    WaitForIdle(BUSY_WAIT, (char*)"end WriteRegister", false);
  }
}
...
void SX126x::ReadRegister(uint16_t reg, uint8_t* data, uint8_t numBytes, bool waitForBusy) {
  // ensure BUSY is low (state meachine ready)
  WaitForIdle(BUSY_WAIT, (char*)"start ReadRegister", true);
  ...
  // wait for BUSY to go low
  if(waitForBusy) {
    WaitForIdle(BUSY_WAIT, (char*)"end ReadRegister", false);
  }
}
...
uint8_t SX126x::WriteCommand2(uint8_t cmd, uint8_t* data, uint8_t numBytes, bool waitForBusy) {
  // ensure BUSY is low (state meachine ready)
  WaitForIdle(BUSY_WAIT, (char*)"start WriteCommand2", true);
  ...
  // wait for BUSY to go low
  if(waitForBusy) {
    WaitForIdle(BUSY_WAIT, (char*)"end WriteCommand2", false);
  }
...
void SX126x::ReadCommand(uint8_t cmd, uint8_t* data, uint8_t numBytes, bool waitForBusy) {
  // ensure BUSY is low (state meachine ready)
  WaitForIdle(BUSY_WAIT, (char*)"start ReadCommand", true);
